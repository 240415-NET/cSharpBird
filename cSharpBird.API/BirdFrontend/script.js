document.addEventListener('DOMContentLoaded', () => {
    ...
    createUserButton.addEventListener('click', async () => {
       ...
       submitUserButton.addEventListener('click', async() =>{
        if (createUsername) {
            try {
                const response = await fetch(`http://localhost:PORTNUMBER/Users/Create${createEmail},${createUsername},${createPassword}`);
    ...
    async function fetchUserLists(userId){
        ...
        try{
            const response = await fetch(`http://localhost:PORTNUMBER/Checklists/ListChecklist${userId}`);
